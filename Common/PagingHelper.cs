...
namespace Common
{
    ...
    public static class PagingHelper
    {
        ...
        //        _pageIndex = pageCount;
        //    }
        //    int startNumber = ((_pageIndex - 1) * _pageSize) + 1;
        //    int endNumber = _pageIndex * _pageSize;
        //    //拼接SQL字符串，加上ROW_NUMBER函数进行分页
        //    StringBuilder newSafeSql = new StringBuilder();
        //    newSafeSql.Append("SELECT T1.*, ROWNUM AS ROWNUMBER ");
        //    newSafeSql.Append(" FROM( " + _safeSql);
        //    newSafeSql.AppendFormat(" ORDER BY {0} ) T1", _orderField);
        //    newSafeSql.AppendFormat(" WHERE ROWNUM <= {0} ", endNumber);
        //   //拼接成最终的SQL语句
        //   StringBuilder sbSql = new StringBuilder();
        //    sbSql.Append("SELECT * FROM (");
        //    sbSql.Append(newSafeSql.ToString());
        //    sbSql.Append(") T2");
        //    sbSql.AppendFormat(" WHERE T2.ROWNUMBER >= {0} ", startNumber);
        ...
        public static string CreatePagingSql(int _recordCount, int _pageSize, int _pageIndex, string _safeSql, string _orderField)
        {
            ...
            //    _pageIndex = pageCount;
            //}
            int startNumber = ((_pageIndex - 1) * _pageSize);

            //拼接SQL字符串，加上ROW_NUMBER函数进行分页
            StringBuilder newSafeSql = new StringBuilder();
            newSafeSql.Append("SELECT T1.* ");
            newSafeSql.Append(" FROM( " + _safeSql);
            newSafeSql.AppendFormat(" ORDER BY {0} ) T1", _orderField);
            newSafeSql.AppendFormat(" LIMIT {0},{1} ", startNumber, _pageSize);

            return newSafeSql.ToString();
        }
        /// <summary>
        /// 获取记录总数SQL语句
        /// </summary>
        ...
        {
            return string.Format(" SELECT COUNT(1) AS RecordCount FROM ({0}) AS T1", _safeSql);
        }
    }
}
