    ...
    $.extend($.jgrid, {
        ...
        getRegional: function (inst, param, def_val) {
            ...
            if (inst.p && inst.p.regional && $.jgrid.regional) {
                ...
                ret = $.jgrid.getAccessor($.jgrid.regional || {}, param);
            }
            if (ret === undefined) {
        ...
        return this.each(function () {
            ...
            var sortkeys = ["shiftKey", "altKey", "ctrlKey"],
                ...
                addCell = function (rowId, cell, pos, irow, srvr, rdata) {
                    ...
                    return "<td role=\"gridcell\" " + prp + "> <span class=\"ui-td-div\">" + (v == "&#160;" ? "" : v) + "</span></td>";
                },
                addMulti = function (rowid, pos, irow, checked, uiclass) {
                    //var v = "<div class=\"ckbox\"><input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + t.p.id + "_" + rowid + "\"><label for=\"jqg_" + t.p.id + "_" + rowid + "\"></label></div>";
                ...
                beginReq = function () {
                    //自动计算列宽度（必须只能有一个autowidth: true）
                    var $gview = $("#gview_" + ts.p.id);
                    if ($gview.attr("load") == undefined) {
                        $gview.attr("load", true);
                        var $autowidth = $gview.find("div.ui-jqgrid-hdiv").find("[autowidth='true'],[autowidth='100%']");
                        var hdivWidth = ts.p.width == undefined ? $gview.width() : ts.p.width
                        // todo
                        //var hboxWidth = $gview.is(":visible") == true ? $gview.find("table#" + ts.p.id).innerWidth() : ($gview.find(".ui-jqgrid-hbox").actual("innerWidth") - 20);
                        var hboxWidth = $gview.is(":visible") == true ? $gview.find("table#" + ts.p.id).innerWidth() : ($gview.find(".ui-jqgrid-hbox").actual("innerWidth") - 20);
                        if ($autowidth.length == 1 && hdivWidth > hboxWidth) {
                            //var _width = parseInt(hdivWidth) - (hboxWidth - $autowidth.innerWidth()) - 100;
                            var _width = parseInt(hdivWidth) - (hboxWidth - $autowidth.innerWidth()) - 25;
                            if (ts.p.height == "100%" || $autowidth.attr("autowidth") == "100%") {
                                //_width = _width ;
                                _width = _width + 20;
                                $gview.find("div.ui-jqgrid-bdiv").css("overflow", "hidden");
                                //$gview.find("div.ui-jqgrid-bdiv").css("border-top", "none").find("table").css("border-top", "1px solid #ddd");
                            }
                            $autowidth.width(_width);
                            $gview.find("table#" + ts.p.id).find("tr.jqgfirstrow").find("td:eq(" + $autowidth.index() + ")").width(_width);
                            $gview.find("div.ui-jqgrid-sdiv").find("tr.footrow").find("td:eq(" + $autowidth.index() + ")").width(_width);
                            ts.grid.headers[$autowidth.index()].width = _width;
                        } else {
                            //$gview.find("div.ui-jqgrid-bdiv").css("border-top", "none").find("table").css("border-top", "1px solid #ddd");
                        }
                    }
                    var $this = $("#" + ts.p.id);
                    // todo
                    //if (ts.p.pagination == true) {
                    if (ts.p.pagination == true || ts.p.viewrecords == true) {
                        //自定义分页
                        if ($this.parents('.ui-jqgrid').find('.gridPager').length == 0) {
                            $this.parents('.ui-jqgrid').append("<div class=\"gridPager\"><div class=\"pagination_info\"></div></div>");
                        }
                    }
                    window.setTimeout(function () {
                        $("#load_" + ts.p.id).css("left", ($this.parents('.ui-jqgrid').width() - $("#load_" + ts.p.id).width()) / 2 - 40);
                        $("#load_" + ts.p.id).css("top", ($this.parents('.ui-jqgrid').height() - $("#load_" + ts.p.id).height()) / 2 - 40);
                        $("#load_" + ts.p.id).show();
                ...
                endReq = function () {
                    ...
                    //自定义分页
                    if (ts.p.pagination == true) {
                        var param = $this.getGridParam();
                        var _totalCount = $this.getGridParam("records");    //总数
                        var _pageSize = $this.getGridParam("rowNum");       //页大小
                        var _pageIndex = $this.getGridParam("page");        //当前页
                        var _costtime = Number($this.getGridParam("costtime")) + Number($this.getGridParam("totaltime"));    //查询耗时
                        var $pageinfo = $("<div class=\"pagination_info\"></div>").html('显示 ' + (((_pageIndex - 1) * _pageSize) + 1) + ' - ' + (_pageIndex) * _pageSize + ' 条/共 ' + _totalCount + ' 条，耗时 ' + _costtime + ' ms');
                        var $page = $("<div class=\"m-pagination\"></div>");
                        $page.page({
                            total: _totalCount == 0 ? 1 : _totalCount,
                            pageIndex: _pageIndex - 1,
                            pageSize: _pageSize,
                            pageSizeItems: ts.p.rowList,
                            pageBtnCount: ts.p.visiblePages,
                            showPageSizes: true,
                            firstBtnText: "首页",
                            lastBtnText: "尾页",
                            prevBtnText: "上一页",
                        ...
                        $page.on("pageClicked", function (event, pageIndex) {
                            //上页-下页
                            $this.jqGrid('setGridParam', { page: pageIndex + 1 }).trigger('reloadGrid');
                        }).on('jumpClicked', function (event, pageIndex) {
                            //跳转页
                            $this.jqGrid('setGridParam', { page: pageIndex + 1 }).trigger('reloadGrid');
                        }).on('pageSizeChanged', function (event, pageSize) {
                            //页大小
                            $this.jqGrid('setGridParam', { page: 1, rowNum: pageSize }).trigger('reloadGrid');
                        });
                        $this.parents('.ui-jqgrid').find('.gridPager').html("");
                        $this.parents('.ui-jqgrid').find('.gridPager').append($page);
                        $this.parents('.ui-jqgrid').find('.gridPager').append($pageinfo);
                    }
                    ...
                    if (ts.p.viewrecords == true) {
                        ...
                        var _costtime = Number($this.getGridParam("costtime")) + Number($this.getGridParam("totaltime"));    //查询耗时
                        //var $pageinfo = $("<div class=\"pagination_info\"></div>").html('共 ' + _totalCount + ' 条，耗时 ' + _costtime + ' ms');
                        var $pageinfo = $("<div class=\"pagination_info\"></div>").html('显示 ' + 1 + ' - ' + _totalCount + ' 条/共 ' + _totalCount + ' 条，耗时 ' + _costtime + ' ms');
                    ...
                    if (ts.p.unwritten) {
                        $this.parents('.ui-jqgrid-bdiv').find(".unwritten").remove();
                        if ($this.find('tbody tr').length == 1) {
                            var $div = $('<div class="unwritten grid-no-entities"><p>' + ts.p.notext + '</p></div>');
                            $div.css("left", ($this.parents('.ui-jqgrid').width() - 300) / 2)
                            $this.parents('.ui-jqgrid-bdiv').append($div);
                        }
                    }
                    window.setTimeout(function () {
                        $("#load_" + ts.p.id).hide();
                        ...
                        switch (dt) {
                            ...
                            case "script":
                                $.ajax($.extend({
                                    ...
                                    success: function (dataJson, st, xhr) {
                                        //debugger;
                                        ts.p.costtime = new Date() - ts.p.costtime;
                                        //todo
                                        if (dataJson.status != 200) {
                                            jinkai.ajaxError(dataJson.msg);//todo
                                            return;
                                        }
                                        // todo
                                        var data = dataJson.data;
                                        //var data = dataJson.data.items != undefined ? dataJson.data.items : dataJson.data;                       
            ...
            //liu --root:"rows" 改为 root:"items" 2021年04月28日
            ts.p.jsonReader = $.extend(true, {
                ...
                id: "id",
                userdata: "userdata",
                subgrid: { root: "items", repeatitems: true, cell: "cell" }
            }, ts.p.jsonReader);
            ...
            imgs += "<span sort='asc'  class='ui-grid-ico-sort ui-icon-asc" + iac + " ui-sort-" + dir + " " + disabled + "  fa fa-sort-asc'></span>";
            imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc" + idc + " ui-sort-" + dir + " " + disabled + " fa fa-sort-desc'></span></span>";
            if (ts.p.multiSort) {
                if (ts.p.sortname) {
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                ...
                thead += "<th " + (tmpcm.autowidth == undefined ? "" : "autowidth='" + tmpcm.autowidth + "'") + " id='" + ts.p.id + "_" + tmpcm.name + "' style='width:" + tmpcm.width + "px;text-align:" + tmpcm.align + ";' role='columnheader' " + getstyle(stylemodule, 'headerBox', false, "ui-th-column ui-th-" + dir) + " " + tooltip + ">";
                idn = tmpcm.index || tmpcm.name;
                // todo 
                if (tmpcm.required != undefined && tmpcm.required == true) {
                    thead += "<div class='ui-th-div' id='jqgh_" + ts.p.id + "_" + tmpcm.name + "' " + tdc + ">" + ts.p.colNames[i] + "<i style='color:red;'> *</i>";
                } else {
            ...
            var firstr = "<tr class='jqgfirstrow' role='row'>";
            ...
            $("th", thr).each(function (j) {
                ...
                if (!(nm === 'cb' || nm === 'subgrid' || nm === 'rn')) {
                    if (ts.p.viewsortcols[2] && tmpcm.sortable == true) {
            ...
            var hTable = $("<table " + getstyle(stylemodule, 'headerTable', false, 'ui-jqgrid-htable ui-common-table') + " style='width:" + ts.p.tblwidth + "px' role='presentation' aria-labelledby='gbox_" + this.id + "'></table>").append(thead),
                ...
                bstw = ts.p.styleUI === 'Bootstrap' ? 0 : 0;
            //todo
            ...
            return this.each(function () {
                ...
                if (this.grid && typeof newParams === 'object') {
                    if (overwrite === true) {
                        ...
                        newParams.page = newParams.page == undefined ? 1 : newParams.page;
            ...
            return this.each(function () {
                ...
                if (!$t.p.multiselect) {
                    if (pt.className !== "ui-subgrid") {
                        if ($t.p.selrow !== pt.id) {
                            $($t).find("tr").removeClass(highlight).attr({ "aria-selected": "false", "tabindex": "-1" });
                            $(pt).addClass(highlight).attr({ "aria-selected": "true", "tabindex": "0" });
                            if (fid) {
                            ...
                            $('td[role="gridcell"]', ind).each(function (i) {
                                ...
                                if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
                                    if ($t.p.treeGrid === true && nm === $t.p.ExpandColumn) {
                                        ...
                                        try {
                                            res[nm] = $.trim($.unformat.call($t, this, { rowId: ind.id, colModel: $t.p.colModel[i] }, i).replace(/<(\S*?)[^>]*>/ig, ""));
                                        } catch (e) {
                    ...
                    while (k < datalen) {
                        ...
                        for (i = gi + si + ni; i < t.p.colModel.length; i++) {
                            ...
                            if (t.p.cellEdit == true) {
                                ...
                                row[row.length] = "<td role=\"gridcell\" " + prp + "><span class='ui-td-div'>" + v + "</span></td>";
                            }
                        }
        ...
        setGridWidth: function (nwidth, shrink) {
            return this.each(function () {
                ...
                var $t = this, cw,
                    ...
                    //bstw = $t.p.styleUI === 'Bootstrap' ? 2 : 0;
                    // todo
                    bstw = $t.p.styleUI === 'Bootstrap' ? 0 : 0;
                    ...
                    if (ln && ln > 0) {
                        while (i < ln) {
                            if ($($t.rows[i]).hasClass('jqgrow')) {
                                try {
                                    var _val = $.unformat.call($t, $($t.rows[i].cells[pos]), { rowId: $t.rows[i].id, colModel: $t.p.colModel[pos] }, pos);
                                    var $span = $(_val);
                                    val = $span.length == 0 ? _val : $span.text();
                                } catch (e) {
        ...
        setFrozenColumns: function () {
            return this.each(function () {
                ...
                if (maxfrozen >= 0 && frozen) {
                    ...
                    $t.grid.fbDiv = $('<div style="position:absolute;' + ($t.p.direction === "rtl" ? 'right:0;' : 'left:0;') + 'top:' + (parseInt(top, 10) + parseInt(hth, 10) + 1) + 'px;overflow-y:hidden;border-top: 0px none !important;" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
                ...
                if (cm.editable === true && ed === true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t, nm, iRow, iCol))) {
                    ...
                    $("input, select, textarea", cc).bind("keydown", function (e) {
                        ...
                        if (e.keyCode === 13 && !e.shiftKey) {
                            ...
                            $($t.rows[$t.p.iRow]).find("td:eq(" + $t.p.iCol + ")").removeClass("edit-cell success");
                            return false;
                        } //Enter
                        if (e.keyCode === 9) {
                            if (!$t.grid.hDiv.loading) {
                    ...
                    if (v2 !== $t.p.savedRow[fr].v) {
                        ...
                        if (cv[0] === true) {
                            ...
                            try {
                                ...
                                // TODO
                                //window.setTimeout(function () { $.jgrid.info_dialog("错误提示",  cv[1], "关闭", { styleUI: $t.p.styleUI, top: p.top + 40, left: p.left }); }, 100);
                                $($t).jqGrid("restoreCell", iRow, iCol);
                            } catch (e) { }
                        }
                    } else {
    ...
    $.jgrid.extend({
        ...
        setGroupHeaders: function (o) {
            ...
            return this.each(function () {
                ...
                if (o.useColSpanStyle) {
                    $htable.find("div.ui-th-div:not(.ui-jqgrid-sortable)").css({
                        "height": "72px",
                        "line-height": "72px"
                    });
        ...
        setTreeNode: function (i, len) {
            return this.each(function () {
                ...
                while (i < len) {
                    ...
                    twrap = "<div class='tree-wrap tree-wrap-" + $t.p.direction + "' style='width:" + (ident * 14) + "px;'>";
                    twrap += "<div style='" + ($t.p.direction === "rtl" ? "right:" : "left:") + (lftpos * 14) + "px;' class='" + common.icon_base + " ";
